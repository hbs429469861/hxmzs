...
<!-- Content area -->
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-framed">
                    ...
                    <tbody>
                     ...
                     <c:forEach items="${mottoEntityList}" var ="c">
                            <tr>
                                <td>${c.id}</td>
                                <td>
                                ...
                                <td>
                                        ${fn:substring(c.mottoAuthor, 0, 5)}...
                                </td>
                                ...
                                <td><a href="#"><i class="icon-file-eye"  data-id="${c.id}" onclick="showMotto(this)" ></i></a></td>
                                <td><a href="#"><i class="icon-pencil7"   data-id="${c.id}" onclick="editMottoUI(this)" ></i></a></td>

                                <td><a href="#"><i class="icon-eraser2 bg-warning-500" data-status="${c.status}"  data-id="${c.id}" <c:if test="${c.status=='1'}">onclick="deleteMotto(this)"</c:if> ></i></a></td>
                            </tr>
                        </c:forEach>
                    </tbody>
                </table>
    ...
    <script type="text/javascript">
        $("#previousPage").click(function() {
            ...
            $('.content').load("${pageContext.request.contextPath}/motto/add2MottoList?pId="+pids);
        });

        $("#nextPage").on('click',function() {
            var pid = $("#nextPage").attr("pid");

            $('.content').load("${pageContext.request.contextPath}/motto/add2MottoList?pId="+pid);
        });
        ...
        /*查看*/
        function showMotto(obj){
            var id=obj.getAttribute("data-id");
            console.log(id);
            $(".content").load("${pageContext.request.contextPath}/motto/show2MottoUI?id="+id);

        }

        /*编辑*/
        function editMottoUI(obj){
            var id=obj.getAttribute("data-id");
            console.log(id);
            $(".content").load("${pageContext.request.contextPath}/motto/edit2MottoUI?id="+id);

        }

        /*删除*/
        function deleteMotto(obj){
            var id = obj.getAttribute("data-id");
            var status = obj.getAttribute("data-status");
            console.log(id);
            $.ajax({
                url : "/motto/deleteById",
                type : "post",
                data : {
                    id:id,
                    status:status
